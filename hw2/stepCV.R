...
#На самом деле в этой функции всё ещё вычисляется и выводится AIC для предикторов,
#но на каждом шаге сравнивается то, что возвращает extractCV, так что функция stepCV
#ведёт себя так, как ожидается.

extractCV <- function(fit, scale, k = 2, ...) UseMethod("extractCV")

extractCV.lm <- function(fit, scale, k = 2, ...) {
  t <- tune(lm, fit$call$formula, data = eval(fit$call$data))
  c(t$best.model$rank, t$best.performance)
}

extractCV.glm <- function(fit, scale, k = 2, ...) {
  t <- tune(glm, fit$call$formula, data = eval(fit$call$data))
  c(t$best.model$rank, t$best.performance)
}

stepCV <- function (object, scope, scale = 0, direction = c("both", "backward", "forward"),
